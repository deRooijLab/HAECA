...
setwd(work_dir)
output_dir <- file.path("~HAECA/global_analysis/downsampling/output_median_parallel")
...
future_lapply(tissues, function(tissue) {
  ...
  target_cells_per_donor <- max(
    10,
    floor(quantile(donor_count_df$CellCount, 0.25))
  )
  ...
  return(TRUE)
    ...
    message("Error in tissue ", t, ": ", conditionMessage(e))
